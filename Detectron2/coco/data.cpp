...
namespace Detectron2
{
  void ImgData::print(std::ostream &os){
    ...
    os << "scale_factor: " << scale_factor_w << "\n";
    os << "scale_factor: " << scale_factor_h << "\n";
  }
  ...
  cv::Mat rescale_image(cv::Mat img, float scale_w, float scale_h){
    cv::Mat out;
    cv::resize(img, out, cv::Size(), scale_w, scale_h, cv::INTER_LINEAR);
    return out;
  }
  ...
  std::tuple<cv::Mat, float, float> rescale_image(cv::Mat img, float img_scale){
    auto h = img.rows;
    auto w = img.cols;
    auto scale_h = img_scale / h;
    auto scale_w = img_scale / w;
    auto rescaled = rescale_image(img, scale_w, scale_h);
    return std::make_tuple(rescaled, scale_w, scale_h);
  }
  ...
  */
  void CocoDataset::transform(ImgData &img_data){
    ...
    // resize
    auto scaled = rescale_image(img, _max_size);
    img = std::get<0>(scaled);
    img_data.scale_factor_w= std::get<1>(scaled);
    img_data.scale_factor_h = std::get<2>(scaled);

    int box_num = img_data.gt_bboxes.size(0);
    int box_num_size = img_data.gt_bboxes.size(1);
    std::vector<torch::Tensor> m_gt_bboxes;
    auto m_gt_bboxes_tensor = img_data.gt_bboxes.clone();
    for (int i_box_num = 0; i_box_num < box_num; i_box_num++)
    {
        std::vector<int32_t> bbox;
        auto b = tolist(m_gt_bboxes_tensor[i_box_num]);
        bbox.push_back((int)b[0].item<float>() * img_data.scale_factor_w);
        bbox.push_back((int)b[1].item<float>() * img_data.scale_factor_h);
        bbox.push_back((int)b[2].item<float>() * img_data.scale_factor_w);
        bbox.push_back((int)b[3].item<float>() * img_data.scale_factor_h);
        auto bbox_tsr = torch::tensor(bbox);
        m_gt_bboxes.push_back(bbox_tsr);
    }
    auto l_gt_bboxes = torch::cat(m_gt_bboxes).view({ box_num, 4 });
    img_data.gt_bboxes.reset();
    img_data.gt_bboxes = l_gt_bboxes;
    img_data.gt_bboxes = img_data.gt_bboxes * 1.0;
    //std::cout << "img_data.gt_bboxes:" << img_data.gt_bboxes << std::endl;
