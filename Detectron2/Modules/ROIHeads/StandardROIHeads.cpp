...
std::tuple<InstancesList, TensorMap> StandardROIHeadsImpl::forward(const ImageList &images, const TensorMap &features,
	...
	if (is_training()) {
		InstancesList results; TensorMap losses;
		tie(losses, results) = _forward_box(features, proposals, targets);
		// Usually the original proposals used by the box head are used by the mask, keypoint
		// heads. But when `m_train_on_pred_boxes is True`, proposals will contain boxes
		// predicted by the box head.
		auto loss_mask = get<0>(_forward_mask(features, proposals));
		update(losses, loss_mask);
		auto loss_keypoint = get<0>(_forward_mask(features, proposals));
		update(losses, loss_keypoint);
...
std::tuple<TensorMap, InstancesList> StandardROIHeadsImpl::_forward_box(const TensorMap &features_,
	...
	auto  m_proposal_box = proposals.getTensorVec("proposal_boxes");
	auto  m_objectness_logits = proposals.getTensorVec("objectness_logits");

	//cout << "predictions[0]:" << predictions[0] << endl;
	//cout << "m_objectness_logits:" << m_objectness_logits << endl;
	if (is_training()) {
		auto losses = m_box_predictors[0]->losses(predictions, proposals, targets);
