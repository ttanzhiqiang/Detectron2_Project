...
std::tuple<torch::Tensor, torch::Tensor> ROIHeadsImpl::_sample_proposals(const torch::Tensor &matched_idxs,
	...
	if (gt_classes.numel() > 0) {
		auto matched_idxs_clone = torch::clone(matched_idxs);
		for (int i_gt_classes = 0; i_gt_classes < gt_classes.numel(); i_gt_classes++)
		{
			matched_idxs_clone.index_put_({ matched_idxs_clone == i_gt_classes }, gt_classes[i_gt_classes]);
		}
		gt_classes = matched_idxs_clone;
...
InstancesList ROIHeadsImpl::label_and_sample_proposals(InstancesList &proposals, const InstancesList &targets) {
	...
	auto &storage = get_event_storage();
	auto mean_num_fg_samples = std::accumulate(std::begin(num_fg_samples), std::end(num_fg_samples), 0.0) / num_fg_samples.size();
	auto mean_num_bg_samples = std::accumulate(std::begin(num_bg_samples), std::end(num_bg_samples), 0.0) / num_bg_samples.size();
	storage.put_scalar("roi_head/num_fg_samples", torch::tensor(mean_num_fg_samples).item<float>());
	storage.put_scalar("roi_head/num_bg_samples", torch::tensor(mean_num_bg_samples).item<float>());
...
InstancesPtr ROIHeadsImpl::add_ground_truth_to_proposals_single_image(const torch::Tensor& gt_boxes,
	...
	auto n_size = proposals->size() + gt_proposal->size();
	//auto new_proposals = proposals->cat({ proposals, gt_proposal });
