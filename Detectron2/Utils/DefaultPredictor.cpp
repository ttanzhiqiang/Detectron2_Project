...
InstancesPtr DefaultPredictor::predict(torch::Tensor original_image) {
	...
	torch::Device _device = torch::Device(DeviceType::CUDA, 0);
	std::vector<DatasetMapperOutput> inputs(1);
	inputs[0].image = image.to(_device);
