...
std::tuple<InstancesList, TensorMap> GeneralizedRCNNImpl::forward(
	...
	InstancesList results; TensorMap detector_losses;
	tie(results, detector_losses) = m_roi_heads(images, features, proposals, gt_instances);
	if (m_vis_period > 0) {
		assert(false);
		auto &storage = get_event_storage();
