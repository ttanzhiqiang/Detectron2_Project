...
Detectron2::subsample_labels(const torch::Tensor &labels, int num_samples, float positive_fraction, int bg_label) {
	...
	auto perm1 = torch::randperm(positive.numel(), positive.device()).index({ Slice(None, num_pos) }).toType(torch::kLong);
	auto perm2 = torch::randperm(negative.numel(), negative.device()).index({ Slice(None, num_neg) }).toType(torch::kLong);
